...
def print_formatted(number):
    numoctstr = len(str(bin(number))) - 2
    for i in range(1, number+1):
        oc = (str(oct(i))).replace("0o","")
        he = (str(hex(i)).upper()).replace("0X","")
        bi = (str(bin(i))).replace("0b","")
        print(" "*(numoctstr - len(str(i))) + str(i), " "*(numoctstr - len(oc))+oc, " "*(numoctstr - len(he))+he, " "*(numoctstr - len(bi))+bi)
